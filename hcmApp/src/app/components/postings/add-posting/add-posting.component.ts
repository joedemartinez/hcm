...
export class AddPostingComponent {
  ...
  ngOnInit():void{
    ...
    if (this.id) {
    this.httpService.get("http://localhost:8080/api/postings/"+this.id).subscribe((results: any) => {
  ...
  submitForm(){
    ...
    if(this.id > ''){ //updating posting
      ...
      this.httpService.put("http://localhost:8080/api/postings/update/"+this.id, this.addPosting.value).subscribe((results: any) => {
      ...
      //make http post request
      this.httpService.post("http://localhost:8080/api/postings/add", this.addPosting.value).subscribe((results: any) => {
