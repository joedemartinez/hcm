...
export class AddUnitComponent {
  ...
  ngOnInit(){
    ...
    if (this.id) {
    this.httpService.get("http://localhost:8080/api/units/"+this.id).subscribe((results: any) => {
  ...
  submitForm(){
    ...
    if(this.id > ''){
      ...
      this.httpService.put("http://localhost:8080/api/units/update/"+this.id, this.addUnit.value).subscribe((results: any) => {
      ...
      //make http post request
        this.httpService.post("http://localhost:8080/api/units/add", this.addUnit.value).subscribe((results: any) => {
